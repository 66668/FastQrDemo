...
public class ContinueCameraPreview extends SurfaceView implements SurfaceHolder.Callback {
    private Camera mCamera;
    private boolean mPreviewing = true;
    private boolean mSurfaceCreated = false;
    private boolean mIsTouchFocusing = false;
    private float mOldDist = 1f;
    private ContinueCameraConfigurationManager mCameraConfigurationManager;

    public ContinueCameraPreview(Context context) {
        super(context);
    }
    ...
    public void setCamera(Camera camera) {
        mCamera = camera;
        if (mCamera != null) {
            mCameraConfigurationManager = new ContinueCameraConfigurationManager(getContext());
    ...
    void onScanBoxRectChanged(Rect scanRect) {
        ...
        ContinueBGAQRCodeUtil.printRect("转换前", scanRect);
        ...
        ContinueBGAQRCodeUtil.printRect("转换后", scanRect);

        ContinueBGAQRCodeUtil.d("扫码框发生变化触发对焦测光");
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getPointerCount() == 1 && (event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP) {
            ...
            ContinueBGAQRCodeUtil.d("手指触摸触发对焦测光");
        ...
        if (event.getPointerCount() == 2) {
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                case MotionEvent.ACTION_POINTER_DOWN:
                    mOldDist = ContinueBGAQRCodeUtil.calculateFingerSpacing(event);
                    break;
                case MotionEvent.ACTION_MOVE:
                    float newDist = ContinueBGAQRCodeUtil.calculateFingerSpacing(event);
                    if (newDist > mOldDist) {
    ...
    private static void handleZoom(boolean isZoomIn, Camera camera) {
        ...
        if (params.isZoomSupported()) {
            ...
            if (isZoomIn && zoom < params.getMaxZoom()) {
                ContinueBGAQRCodeUtil.d("放大");
                zoom++;
            } else if (!isZoomIn && zoom > 0) {
                ContinueBGAQRCodeUtil.d("缩小");
                zoom--;
            } else {
                ContinueBGAQRCodeUtil.d("既不放大也不缩小");
            }
            params.setZoom(zoom);
            camera.setParameters(params);
        } else {
            ContinueBGAQRCodeUtil.d("不支持缩放");
        }
    }
    ...
    private void handleFocusMetering(float originFocusCenterX, float originFocusCenterY,
        ...
        try {
            ...
            if (focusMeteringParameters.getMaxNumFocusAreas() > 0) {
                ContinueBGAQRCodeUtil.d("支持设置对焦区域");
                isNeedUpdate = true;
                Rect focusRect = ContinueBGAQRCodeUtil.calculateFocusMeteringArea(1f,
                        originFocusCenterX, originFocusCenterY,
                        originFocusWidth, originFocusHeight,
                        size.width, size.height);
                ContinueBGAQRCodeUtil.printRect("对焦区域", focusRect);
                ...
                ContinueBGAQRCodeUtil.d("不支持设置对焦区域");
            }

            if (focusMeteringParameters.getMaxNumMeteringAreas() > 0) {
                ContinueBGAQRCodeUtil.d("支持设置测光区域");
                isNeedUpdate = true;
                Rect meteringRect = ContinueBGAQRCodeUtil.calculateFocusMeteringArea(1.5f,
                        originFocusCenterX, originFocusCenterY,
                        originFocusWidth, originFocusHeight,
                        size.width, size.height);
                ContinueBGAQRCodeUtil.printRect("测光区域", meteringRect);
                focusMeteringParameters.setMeteringAreas(Collections.singletonList(new Camera.Area(meteringRect, 1000)));
            } else {
                ContinueBGAQRCodeUtil.d("不支持设置测光区域");
            }
            ...
            if (isNeedUpdate) {
                ...
                mCamera.autoFocus(new Camera.AutoFocusCallback() {
                    public void onAutoFocus(boolean success, Camera camera) {
                        if (success) {
                            ContinueBGAQRCodeUtil.d("对焦测光成功");
                        } else {
                            ContinueBGAQRCodeUtil.e("对焦测光失败");
                        }
                        startContinuousAutoFocus();
                    }
                });
            ...
            e.printStackTrace();
            ContinueBGAQRCodeUtil.e("对焦测光失败：" + e.getMessage());
            startContinuousAutoFocus();
        }
    }
...
//            parameters.setFocusMode(Camera.Parameters.FOCUS_MODE_MACRO);//TODO
            ...
            ContinueBGAQRCodeUtil.e("连续对焦失败");
        }
    }
